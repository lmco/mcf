...
describe(M.getModuleName(module.filename), () => {
  ...
  before(async () => {
    try {
      ...
      artifacts = await ArtifactController.create(adminUser, org.id, projID, branchID, arts);
    }
    catch (error) {
      M.log.error(error);
      // Expect no error
  ...
  it('should return a raw JSON version of an artifact instead of a document with'
...
async function createArchivedArtifact() {
  try {
    ...
    // Create the artifact
    const createdArtifacts = await ArtifactController.create(adminUser, org.id, projID,
      branchID, artObj);
...
async function archiveArtifact() {
  try {
    ...
    const updatedArtifacts = await ArtifactController.update(adminUser, org.id, projID,
      branchID, updateObj);
...
async function optionPopulateFind() {
  ...
  // Get the artifact ID
  const artID = utils.parseID(artifacts[0].id).pop();

  try {
    // Find an artifact using the populate option
    const foundArtifacts = await ArtifactController.find(adminUser, org.id, projID,
    ...
    pop.forEach((field) => {
      // If the field is defined in the returned artifact
      if (art.hasOwnProperty(field)) {
        ...
        chai.expect(art.field.hasOwnProperty('_id')).to.equal(true);
      }
    });
  }
  catch (error) {
...
async function optionArchivedFind() {
  try {
    const artID = utils.parseID(artifacts[3].id).pop();

    // Create the options object
    const options = { archived: true };

    // Attempt to find the artifact without providing options
    const notFoundArtifacts = await ArtifactController.find(adminUser, org.id, projID, branchID,
      artID);
    ...
    const foundArtifacts = await ArtifactController.find(adminUser, org.id, projID, branchID,
      artID, options);
...
async function optionFieldsFind() {
  try {
    ...
    const visibleFields = Object.keys(art._doc);
    ...
    const notFindArtifacts = await ArtifactController.find(adminUser, org.id, projID,
    ...
    // Create a list of visible artifact fields. Object.keys(art) returns hidden fields as well
    const visibleFields2 = Object.keys(art2._doc);
...
async function optionSkipFind() {
  try {
    ...
    const secondArtifacts = await ArtifactController.find(adminUser, org.id, projID, branchID,
      secondOptions);
    // Verify that no more than 2 artifacts were found
 ...
 * artifact rather than a document with instance methods in the find() function.
 */
async function optionLeanFind() {
  try {
    ...
    // Find the artifact without the lean option
    const notLeanarts = await ArtifactController.find(adminUser, org.id, projID,
      branchID, artID);
    // Expect there to be exactly 1 artifact found
    chai.expect(notLeanarts.length).to.equal(1);
    const art = notLeanarts[0];

    // Expect the instance method getValidUpdateFields to be a function
    chai.expect(typeof art.getValidUpdateFields).to.equal('function');

    // Find the artifact WITH the lean option
    const leanarts = await ArtifactController.find(adminUser, org.id, projID,
      branchID, artID, options);
    // Expect there to be exactly 1 artifact found
    chai.expect(leanarts.length).to.equal(1);
    const art2 = leanarts[0];

    // Expect the instance method getValidUpdateFields to be undefined
    chai.expect(typeof art2.getValidUpdateFields).to.equal('undefined');
  }
  catch (error) {
    M.log.error(error);
    // Expect no error
...
async function optionFieldsCreate() {
  try {
    ...
    const notFindArtifacts = await ArtifactController.create(adminUser, org.id,
...
/**
 * @description Verifies that providing the option 'lean' returns raw JSON of an
 * artifact rather than a document with instance methods object in the create()
 * function.
 */
async function optionLeanCreate() {
  ...
  const notLeanartObj = {
    id: 'not-lean-artifact',
    name: 'Not Lean Artifact',
    location: 'notlean',
    filename: 'notlean.jpeg'
  };
  // Create the options object with lean: true
  const options = { lean: true };

  try {
    // Create the artifact without the lean option
    const createdArtifacts = await ArtifactController.create(adminUser, org.id, projID,
      branchID, notLeanartObj);
    // Expect there to be exactly 1 artifact created
    chai.expect(createdArtifacts.length).to.equal(1);
    const art = createdArtifacts[0];
    ...
    // Create the artifact WITH the lean option
    const createdArtifacts2 = await ArtifactController.create(adminUser, org.id, projID,
      branchID, leanartObj, options);
    // Expect there to be exactly 1 artifact created
    chai.expect(createdArtifacts2.length).to.equal(1);
    const art2 = createdArtifacts2[0];
...
async function optionFieldsUpdate() {
  try {
    ...
    const notFindArtifacts = await ArtifactController.update(adminUser, org.id,
...
/**
 * @description Verifies that providing the option 'lean' returns raw JSON of an
 * artifact rather than a document with instance methods object in the update()
 * function.
 */
async function optionLeanUpdate() {
  try {
    // Create the update object
    const art0 = {
      id: utils.parseID(artifacts[0].id).pop(),
      name: 'update 1'
    };

    // Create the options object with lean: true
    const options = { lean: true };
    // Update the artifact without the lean option
    const updatedArtifacts = await ArtifactController.update(adminUser, org.id,
      projID, branchID, art0);

    // Expect there to be exactly 1 artifact updated
    chai.expect(updatedArtifacts.length).to.equal(1);
    const art1 = updatedArtifacts[0];
    chai.expect(art1.name).to.equal('update 1');

    // Expect the instance method getValidUpdateFields to be a function
    chai.expect(typeof art1.getValidUpdateFields).to.equal('function');

    // Make a second update
    art0.name = 'Batch Artifact 0';

    // Update the artifact WITH the lean option
    const leanUpdatedArtifacts = await ArtifactController.update(adminUser, org.id, projID,
      branchID, art0, options);

    // Expect there to be exactly 1 artifact updated
    chai.expect(leanUpdatedArtifacts.length).to.equal(1);
    const art2 = leanUpdatedArtifacts[0];
    chai.expect(art2.name).to.equal('Batch Artifact 0');
...
async function optionSortFind() {
  try {
    ...
    const createdElems = await ArtifactController.create(adminUser, org.id, projID,
      branchID, testArts);
    ...
    const foundElems = await ArtifactController.find(adminUser, org.id, projID, branchID,
      testArts.map((e) => e.id),
      sortOption);
    ...
    chai.expect(foundElems[0].id).to.equal(utils.createID(org.id, projID, branchID, 'testart02'));
    chai.expect(foundElems[1].name).to.equal('b');
    chai.expect(foundElems[1].id).to.equal(utils.createID(org.id, projID, branchID, 'testart00'));
    chai.expect(foundElems[2].name).to.equal('c');
    chai.expect(foundElems[2].id).to.equal(utils.createID(org.id, projID, branchID, 'testart01'));

    // Find the artifacts and return them sorted in reverse
    const reverseElems = await ArtifactController.find(adminUser, org.id, projID, branchID,
      testArts.map((e) => e.id),
      sortOptionReverse);
    // Expect to find all three artifacts
    chai.expect(foundElems.length).to.equal(3);
    ...
    chai.expect(reverseElems[0].id).to.equal(utils.createID(org.id, projID, branchID, 'testart01'));
    chai.expect(reverseElems[1].name).to.equal('b');
    chai.expect(reverseElems[1].id).to.equal(utils.createID(org.id, projID, branchID, 'testart00'));
    chai.expect(reverseElems[2].name).to.equal('a');
    chai.expect(reverseElems[2].id).to.equal(utils.createID(org.id, projID, branchID, 'testart02'));

    await ArtifactController.remove(adminUser, org.id, projID, branchID,
      testArts.map((e) => e.id));
  }
  catch (error) {
