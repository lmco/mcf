...
/*  const zipDataPromise = new Promise((resolve, reject ) => {
    const zipStream = fs.createReadStream(path.join(M.root, zipFile));
    const chunks = [];
...
/*  const zipPromise = new Promise((resolve, reject) => {
    fs.readFile(path.join(M.root, zipFile), (err, data) => {
  ...
  const zipData = fs.readFileSync(path.join(M.root, zipFile))
