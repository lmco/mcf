...
function createRootPackage(done) {
  // Create unique ID
  const uniqueID = utils.createID(project._id, testData.elements[0].id);
  ...
  .then(() => Element.Package.findOne({ _id: uniqueID }))
  .then((retPackage) => {
    // Check the root package element saved correctly
    chai.expect(retPackage._id).to.equal(uniqueID);
...
function createBlock(done) {
  ...
  .then((_pkg) => {
    ...
    const newBlock = new Element.Block({
      _id: utils.createID(project._id, testData.elements[1].id),
  ...
  .then((createdBlock) => {
    ...
    chai.expect(createdBlock._id).to.equal(
      utils.createID(project._id, testData.elements[1].id)
    );
...
function createRelationship(done) {
  ...
  .then((_pkg) => {
    ...
    const newRelationship = new Element.Relationship({
      _id: utils.createID(project._id, testData.elements[3].id),
  ...
  .then((createdRelationship) => {
    ...
    chai.expect(createdRelationship._id).to.equal(
      utils.createID(project._id, testData.elements[3].id)
    );
...
function findBlock(done) {
  // Find the block
  Element.Element.findOne({ _id: utils.createID(project._id, testData.elements[1].id) })
  .then((element) => {
...
function verifyBlankElemName(done) {
  ...
  const newElement = new Element.Block({
    _id: utils.createID(project._id, testData.elements[5].id),
  ...
  .then((retElement) => {
    ...
    chai.expect(retElement._id).to.equal(
      utils.createID(project._id, testData.elements[5].id)
    );
