...
function getOne(reference) {
  return function(done) {
    let levelData;
    let ref;
    let url = `${test.url}/api/orgs/${org._id}`;
    ...
    if (reference === 'branch') {
      levelData = branchWebhooks;
      ref = utils.createID(org._id, projID, branchID);
      url += `/projects/${projID}/branches/${branchID}`;
    }

    const webhookData = levelData[0];

    url += `/webhooks/${levelData[0]._id}`;
...
function getMany(done) {
  const url = `${test.url}/api/webhooks?server=false`;
