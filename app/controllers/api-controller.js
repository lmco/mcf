    ...
    if (mmsConfig) {
      orgs.forEach(org => {
        ...
        const requestOptions = {
          url: `${req.protocol}://${M.config.server.commitURL}:${port}/plugins/mms3-adapter/alfresco/service/sdvc-org`,
          headers: {
    ...
    if (mmsConfig) {
      projects.forEach(proj => {
        ...
        const projectRequestOptions = {
          url: `${req.protocol}://${M.config.server.commitURL}:${port}/plugins/mms3-adapter/alfresco/service/sdvc-project`,
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${req.session.token}`,
            'SDVC-TOKEN': req.session.sdvc_token
        ...
        axios(projectRequestOptions).then(async function() {
          ...
          elements.forEach(element => {
            const elementRequestOptions = {
              url: `${req.protocol}://${M.config.server.commitURL}:${port}/plugins/mms3-adapter/alfresco/service/sdvc-element`,
              headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${req.session.token}`,
                'SDVC-TOKEN': req.session.sdvc_token
    ...
    if (mmsConfig) {
      ...
      branches.forEach(branch => {
        ...
        const branchRequestOptions = {
          url: `${req.protocol}://${M.config.server.commitURL}:${port}/plugins/mms3-adapter/alfresco/service/sdvc-branch`,
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${req.session.token}`,
            'SDVC-TOKEN': req.session.sdvc_token
