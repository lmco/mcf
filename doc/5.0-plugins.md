...
We must also define out model as JSON in a file called `data.json`. 
...
Now we must create our application entrypoint, the `app.js` file. Create a file
called `app.js` and begin by defining an express application and some 
...
We'll start by defining two routes. The first is a simple redirect for our 
plugin's homepage. We'll replace this later with an actual view, but for now 
we'll just redirect to our API endpoint.
...
Next, we'll add a simple authenticated API endpoint that takes an organization 
ID and a project ID in the URL parameters and identifies this endpoint as a mass 
resource.
...
If alternatively you wanted to use the model instead of the controller, you 
would have to check those permissions yourself to avoid exposing data to a user
that that user should not be able to access. That approach would increase the 
...
actually do something with that data rather to return something more meaningful
...
to create an EJS file that will we can render when showing the home page for 
our plugin.
...
<script>
  ...
  .done(function (msg, status) {
      window.location.replace('/<%= org.id %>');
  })
  .fail(function(msg) {
