...
`npm init` and enter the information for your plugin. Most important is the
...
can be used as follows to find an element with an ID of `model` on the `master`
branch that belongs to the `example` project in the `default` organization:
