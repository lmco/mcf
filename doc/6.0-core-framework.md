...
handling routes, and can require a user is authenticated before access some
...
which use AES-256 encryption to encrypt and decrypt some string.
...
simplify error logging in MBEE. This class is attached to the "M" Object, so it
is recommended that you only access it though that. For example:
```javascript
...
user. These functions are used in API routes, and we recommend using them if
...
object (generally the element object). This format is useful for lookup by that
unique identifier, as look-ups in objects are much faster than in arrays.
...
specifically to log the route and occasionally log the IP.
...
#### Permissions
`permissions.js` provides functions for checking what a user does and doesn't
have permissions to do. While not currently, these functions will be implemented
in a later release.
...
#### Test Utils
`test-utils.js` is a very useful file which provides some functions used to
simplify unit testing. There are functions to create/delete test organizations, 
...
- toTitleCase(string)
  - Title cases a string, meaning it capitalizes the first letter.
...
validation is not passed, the object is not allowed to be created.
