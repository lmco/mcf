...
sed -i "s/--docker-password=dockerpassword/--docker-password=$DockerPassword/" $ProjectDirectory/scripts/pipeline/deploy-test.sh
...
kubectl delete deployment mcf-deployment -n test
