...
function elementHelper() {
  return new Promise((resolve, reject) => {
    ...
    .then((elements) => {
      ...
      // Create an array to store promises, used in Promise.all()
      const promises = [];

      // Find, store and replace elements in batches of 50000
      for (let i = 0; i < elementIDs.length / 50000; i++) {
        const batch = elementIDs.slice(i * 50000, i * 50000 + 50000);
        let elems = [];
        let deleted = false;
        let error = '';
        promises.push(Element.find({ _id: { $in: batch } }).lean()
        .then((foundElements) => {
          elems = foundElements;
          // Write contents to temporary file
          return new Promise(function(res, rej) {
            fs.writeFile(path.join(M.root, 'data', `elements-${i}-081.json`),
        ...
        .then(() => Element.deleteMany({ _id: { $in: batch } }).lean())
        .then(() => {
          deleted = true;
        ...
        .then(() => {
          ...
          if (fs.existsSync(path.join(M.root, 'data', `elements-${i}-081.json`))) {
            return new Promise(function(res, rej) {
              fs.unlink(path.join(M.root, 'data', `elements-${i}-081.json`), function(err) {
                if (err) rej(err);
                else res();
              });
            });
